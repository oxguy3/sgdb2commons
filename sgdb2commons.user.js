...
function makeNavItem() {
    ...
    navSpan.addEventListener('click', function(event) {
        ...
        if (document.querySelector('.asset-container.compact')) { // pages containing a list of images
            let assets = document.querySelectorAll('.asset-container.compact > .asset.selected-2commons');
            if (assets.length == 0) {
                alert('Error: No files selected. Please select all the images you want to upload first. Images can be selected by hovering your mouse over them and clicking on the Commons logo.');
                return;
            }
            for (let i = 0; i < assets.length; ++i) {
                ...
                urls.push(generateUrl(obj, asset, asset.querySelector('.game-name-wrapper .muted')));
            }

        } else if (document.querySelector('.container-asset-page')) { // pages containing a single image
        ...
        } else { // inscrutable pages
            alert('Error: sgdb2commons was not able to recognize any image assets on this page.');
            return;
        }
...
(new MutationObserver(checkPage)).observe(document.getElementById('render-me-uwu'), {childList: true, subtree: true});

function checkPage(changes, observer) {
    if(!isNavLoaded && !document.querySelector('nav .nav-2commons')) {
        makeNavItem();
    }
    if(document.querySelector('.asset-container.compact > .asset .preview .hover-info .lr:not(:has(.btn-2commons))')) {
        addImageButtons();
    }
}